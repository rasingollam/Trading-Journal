...
export async function calculateMetrics(strategyId) {
    const rows = await db.select().from(trades)
        .where(eq(trades.strategyId, strategyId))
        .orderBy(asc(trades.createdAt));
    const results = rows
        .map((r) => r.resultR ? parseFloat(r.resultR.toString()) : null)
        .filter((r) => r !== null);
    if (results.length === 0) {
    ...
    const grossLoss = results.filter((r) => r < 0).reduce((s, r) => s + r, 0);
    ...
    const balanceR = results.reduce((s, r) => s + r, 0);
