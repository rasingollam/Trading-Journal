...
router.get("/:tradeId", async (req, res, next) => {
    try {
        const id = parseInt(req.params.tradeId, 10);
        if (isNaN(id)) {
...
router.post("/", upload.fields([
    ...
    try {
        ...
        const row = await tradesService.createTrade(strategyId, { resultR: req.body.resultR, notes: req.body.notes, pair: req.body.pair }, {
...
router.put("/:tradeId", upload.fields([
    ...
    try {
        ...
        const files = req.files;
        const row = await tradesService.updateTrade(id, { resultR: req.body.resultR, notes: req.body.notes, pair: req.body.pair }, {
            openScreenshot: files?.openScreenshot?.[0],
            closeScreenshot: files?.closeScreenshot?.[0],
        });
        if (!row) {
